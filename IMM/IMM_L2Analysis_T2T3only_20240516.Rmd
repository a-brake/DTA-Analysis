...
```
### Feature plot
```{r plot top10 markers for each cluster, fig.height=25, fig.width=20}
#top_markers_T2T3_res0.3 <- lapply(markers_T2T3_res0.3, filter, pct.1 >= 0.75) # filter by upper and lower cutoff for pct1 and pct2

# use apply to select top 10 markers from each list item
top10_markers_res0.3 <-  lapply(markers_T2T3_res0.3, head, 10)

#markers.to.exp(top10_markers_res0.3,'top10_markers_res0.3')

top10_markers_res0.3.toplot <- rmDup(top10_markers_res0.3)

## FEATURE PLOT ##
n.col = 2 
order.input <- T

fp <- lapply(seuratobj = IMM_T2T3, X = top10_markers_res0.3.toplot[[2]], FUN = FPlist)
...
dp <- lapply(seuratobj = IMM_T2T3, X = top10_markers_res0.3.toplot[[2]], DPlist)

rel.height <- as.numeric(lapply(top10_markers_res0.3.toplot[[2]], length)) #take the expected number of rows and based on length of markers for each cluster and the number of columns

p.name <- 'FeaturePlot_IMMT2T3_RNA_snn_res.0.3_top10' # to use for export
 plot_grid(plotlist = dp, ncol = 3, rel_heights = rel.height, labels = paste0('Cluster ', names(dp)), label_colour = '#ae2012', vjust = 1.0, label_size = 16, scale = 0.85, axis = 'l') #merge plots for all clusters into one big figure for export
...
n.col = 3
order.input = T
FPlist(seuratobj = IMM_T2T3.sub, genes = c('Gm10790', 'mt-Co2,', 'Ifi203', 'Snhg11','Top2a','Sox11','Fmn1'))
FPlist(seuratobj = IMM_T2T3.sub, genes = c('Apoe', 'Igf1', 'Lilrb4', 'Lyz2', 'Colec12', 'Msr1',  'Map1lc3b')) #  shown to be part of the phagocytic signature in adult cerebellar microglia 
```
